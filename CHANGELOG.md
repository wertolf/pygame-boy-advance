...
针对 Card Prisoner 的细节优化
* 按住某键可以触发长按效果，即连续触发 KEYDOWN 事件
  * 在查阅文档的时候意外发现，只需添加 1 行 pygame.key.set_repeat 即可解决问题
  * 当然，我对于它的实现方式也已经有了基本的构想，我已经想到了 delay 和 interval 这两个参数的区别
  * 但是既然 pygame 已经实现了这个功能，就暂时没必要重复造轮子了，并且一般来说库函数的实现会比我自己的实现更加可靠、全面
  * [ ] 之后有时间的时候可以去研究一下 pygame 源码的相应片段
  * 现在的重点只是在游戏中添加相应的功能
* 使用 pygame.key.name 替代所有表示按键的字符串常量
